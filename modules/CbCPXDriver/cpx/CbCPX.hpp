...
private:
    ...
    static constexpr std::size_t can_msg_size = sizeof(bcm_msg_head) + sizeof(can_frame);
    ...
    static constexpr size_t nf(NotifyFlag f) {
        return static_cast<size_t>(f);
    }

    /// @brief Helper to set value of notify_flag bit
    void notify_set(NotifyFlag f, bool value) {
        notify_flags.set(nf(f), value);
    }
