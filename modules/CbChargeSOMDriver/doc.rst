...
with a parameter block which matchs the desired target setup.
...
- RX thread: This thread just cares about receiving the UART frames sent by
  ...
  a difference, it puts the new value into an queue for later processing.
  Then there is the PT1000 temperature frame. Is is also send by the safety
  ...
  The third class of frame are the ones which are only received upon request
  by an inquiry frame. Usually, the requesting function wants to be informed
  about the incoming frame, thus a notify mechanism is used based on condtion
  variables.
...
- TX thread: This thread is responsible to periodically send out the Charge Control
...
in a one second interval from the (cache) structures of `CbChargeSOM` and publishs
