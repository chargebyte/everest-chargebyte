            ...
            for (unsigned int i = 0; i < supported_channels; ++i) {
                ...
                if (flags & PT1000_CHARGING_STOPPED) {
                    if (!this->charging_abort_cause_reported[i]) {
                        ...
                        auto e = this->error_factory->create_error("generic/VendorError",
                                                                   "ChargingAbort/" + t.identification.value(),
                    ...
                    if (this->charging_abort_cause_reported[i]) {
                        ...
                        this->clear_error("generic/VendorError", "ChargingAbort/" + t.identification.value());
                ...
                if (flags & PT1000_SELFTEST_FAILED) {
                    if (!this->selftest_failed_reported[i]) {
                        ...
                        auto e = this->error_factory->create_error("generic/VendorError",
                                                                   "SelftestFailed/" + t.identification.value(),
                                                                   errmsg.str(), Everest::error::Severity::High);
                        this->raise_error(e);
                    ...
                    if (this->selftest_failed_reported[i]) {
                        ...
                        this->clear_error("generic/VendorError", "SelftestFailed/" + t.identification.value());
