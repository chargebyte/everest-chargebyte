...
static const char* const diagnostic_commands[] = {
    ...
    "uptime",
    "timedatectl status",
    "[ -d /sys/bus/i2c/devices/0-0052 ] && hwclock --param-get bsm",
    "cat /sys/kernel/debug/gpio",
    "cat /sys/kernel/debug/pwm",
    "[ -f /sys/kernel/debug/eth1/info ] && cat /sys/kernel/debug/eth1/info",
    "[ -f /sys/kernel/debug/eth2/info ] && cat /sys/kernel/debug/eth2/info",
    "ethtool eth0",
    "ethtool -S eth1",
    "[ -x /usr/bin/plctool ] && plctool -r -i eth1",
    "ethtool eth1",
    "{ ethtool eth1 | grep -q PLCA; } && ethtool --get-plca-cfg eth1",
    "ethtool -S eth2",
    "[ -x /usr/bin/plctool ] && plctool -r -i eth2 FF:FF:FF:FF:FF:FF",
    "[ -x /usr/bin/plctool ] && plcstat -t -i eth2",
    "[ -x /usr/bin/plcID ] && [ \"$(plcID -i eth2 -M)\" = '50:D3:E4:93:3F:85:5B:70:40:78:4D:F8:15:AA:8D:B7' ]"
    " && echo 'Warning: HomePlugAV default key set on mains interface!'",
    "fw_printenv",
