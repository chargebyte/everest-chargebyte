...
void evse_board_supportImpl::init() {
    ...
    if (!this->mod->config.grid_phase_count_gpio_line_name.empty()) {
        if (get_gpioline_state_by_name(this->mod->config.grid_phase_count_gpio_line_name,
                                       this->mod->config.grid_phase_count_active_low, "CbTarragonDriver")) {
            // GPIO indicates three phase wiring
            // so we don't need to adjust max_phase_count and we cannot tell whether
            // phase count switching is supported (keeping the value determined above)
            EVLOG_info << "3-phase wiring mode (phase count switching " << (support_3ph1ph ? "enabled" : "disabled")
                       << ")";
        } else {
        ...
        EVLOG_info << "(default) " << max_phase_count << "-phase wiring mode (phase count switching "
                   << (support_3ph1ph ? "enabled" : "disabled") << ")";
    }
