...
std::ostream& operator<<(std::ostream& os, const error_code& ec) {
    ...
    // we will with 2 places since the datasheet also uses 2 places
...
std::ostream& InfypowerCANID::dump(std::ostream& os) const {
    os << "CAN ID (EC:" << this->get_error_code() << "," << this->get_device_no() << ",CMD:" << this->get_cmd();

    if (this->get_dst_addr() == InfypowerCANID::BROADCAST_ADDR)
        os << ",DST:broadcast";
    else
        os << ",DST:" << this->get_dst_addr();

    if (this->get_src_addr() == InfypowerCANID::BROADCAST_ADDR)
        os << ",SRC:broadcast";
    else
        os << ",SRC:" << this->get_src_addr();
